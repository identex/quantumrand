...
class TestQuantumRandom(unittest.TestCase):
    ...
    def test_uint16_json_api(self):
        data = quantumrand.get_data('uint16', 1, 1)
        assert len(data) == 1
        for i in str(data[0]):
            assert i in '0123456789', i

    def test_uint16_json_api_max_array(self):
        data = quantumrand.get_data('uint16', 100, 100)
        assert len(data) == 100

    def test_hex16_json_api(self):
        data = quantumrand.get_data('hex16', 1, 1)
        assert len(data) == 1
        for h in data[0]:
    ...
    def test_uint16_json_api_long_array(self):
        try:
            data = quantumrand.get_data('uint16', 1025, 1)
    ...
    def test_uint16_json_api_large_blocksize(self):
        try:
            data = quantumrand.get_data('uint16', 1, 1025)
    ...
    def test_uint16_json_api_invalid_type(self):
        try:
            data = quantumrand.get_data('binary', 1, 1)
    ...
    def test_randint(self):
        assert(quantumrand.randint(0, 0) == 0)
        for i in range(5):
            for j in range(i + 1, 5):
                for k in range(3):
                    val = quantumrand.randint(i, j)
